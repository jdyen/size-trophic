...
for (i in seq_along(var_list)) {
  ...
  pd_regress[[i]] <- partial_dependence(mod_ctree1$finalModel, vars = var_list[i],
                                        data = data_pd)
  pd_class[[i]] <- partial_dependence(mod_ctree1_guild$finalModel, vars = var_list[i],
                                      data = data_pd)
}

# fit a Bayesian mixed model of tl against all predictors
mod_stan <- stan_lmer(tl ~ (len | ord) + edhd + mobd + jlhd + cfdcpd + invasive + fresh,
                      data = sp_data,
                      iter = 5000,
                      cores = 4)
...
for (i in seq_len(n_cv)) {
  ...
  # fit lm models
  mod_tmp <- stan_lmer(tl ~ (len | ord) + edhd + mobd + jlhd + cfdcpd + invasive + fresh,
                       data = training,
                       iter = 5000,
                       cores = 4)
...
predictions_stan_converted <- apply(predictions_stan, c(1, 2), convert_tl_to_guild)
predictions_converted <- apply(predictions, c(1, 2), convert_tl_to_guild)
...
write.csv(table1_data, file = "outputs/tables/table1_data.csv")
write.csv(imp_all, file = "outputs/tables/importance_values.csv")
saveRDS(stan_predictions, file = "outputs/stan_predictions.rds")
saveRDS(ctree_predictions, file = "outputs/ctree_predictions.rds")
saveRDS(sp_data, file = "data/full-data-set.rds")
saveRDS(mod_stan, file = "outputs/mod_stan.rds")
saveRDS(pd_regress, file = "outputs/pd_regress.rds")
saveRDS(pd_class, file = "outputs/pd_class.rds")
